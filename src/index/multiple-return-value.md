...
Some languages, such as Go and Lua, let a function return several values directly, separated by commas, without wrapping them in a container. For example, ``func divide(a, b int) (int, int) { return a/b, a%b }``. The caller receives them individually, typically by assigning each to its own variable in one statement.

...
<?php
    ...
    // name collision: function names are case insensitive, so trumpet === TRUMPET
    // function TRUMPET() {}

    // TRUMPET as a constant and TRUMPET as a function are distinct elements.
    const TRUMPET = 1;

    // variable collisions are silent : the variable's value is simply changed
    $trunk = 1;
    foreach($herd as $trunk) {
        print $trunk; // may print a different value than 1
    }

...
function forage() {
    ...
    static $trunk; // No assignation, but $trunk is not NULL, on the first call

    global $trunk; // $trunk is not static anymore, and is 4
    ...
    } catch (Exception $trunk) {
        // $trunk becoms an exception, if it happens
    }

    // finally, $trunk becomes 5, stays global
    $trunk = 5;
}

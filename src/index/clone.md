...
<?php
    ...
    // instantiation
    $original = new Elephpant;

    // cloning
    $twin = clone $original;

    $secondTwin = clone($original);  // parenthesis are now useful
    echo $secondTwin->weight;  // 2
    $customTwin = clone($original, ['weight' => 3]);
    echo $customTwin->weight;

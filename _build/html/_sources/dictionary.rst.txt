...
Catch is the sister clause of try : it defines which exception will be caught, and, optionnaly, which variable will hold it when it happens.
...
+ list separator, where uncaptured variables may be omitted, leaving commas back to back.
...
define() used to create case-insensitive constant, but this was abandonned in PHP 7.3.
...
Extensions are published in the PECL repositor. Some are available by default, in the PHP standard configuration. 
...
Finally is run everytime after a try, with or without any catch clause. It is also run before the return expression, if any. 
...
PHP index may be integers or strings only. Other data type will generate an error or a type conversion. For example, floats will be turned into integer. integer-shaped strings will also be turned into integer.

PHP index start at 0. They may be automatically assigned by appending a new value with the `[]` operator : then, they will use the greatest available index + 1.
...
The magic methods to handle magic properties are :  __get(), __set(), __isset() et __unset().
...
+ The merging of arrays. This merging will set the keys once, then omit the next values.
...
Class properties are variables, local to the class.
...
They might have a typehint, since PHP 7.4
...
In case of doubt, you should read about ``floats``. 
...
The spaceship operator got its name from the ressemblence to an flying saucer. Battleship operator, or death star operator do not exist. 
...
Throwable is a PHP native interface, which is required for any object which will be used with thrown, such as `Exception` and `Error`.

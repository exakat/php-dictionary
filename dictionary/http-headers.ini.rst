...
In PHP, request headers are available in the ``$_SERVER`` superglobal with the ``HTTP_`` prefix, e.g., ``$_SERVER['HTTP_ACCEPT']``, or through ``getallheaders()`` for the full list.
...
Related : :ref:`HTTP Headers <http-header>`, :ref:`Hyper Text Transfer Protocol (HTTP) <http>`, :ref:`Headers <headers>`, :ref:`$_SERVER <$_server>`

...
When ``short_open_tag = Off``, only ``<?php`` and ``<?=`` are valid opening tags. This is the recommended setting and the default since PHP 7.0.

The directive also affects ASP-style tags (``<% %>``), which were removed entirely in PHP 7.0.
...
See also `PHP RFC: Deprecate short open tags, again <https://wiki.php.net/rfc/deprecate_php_short_tags_v2>`_ and `php.ini core directives — short_open_tag <https://www.php.net/manual/en/ini.core.php#ini.short-open-tag>`_.

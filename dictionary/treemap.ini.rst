.. _treemap:
.. meta::
	:description:
		Treemap: Treemaps are a visualization tool used to understand the relative weight of partitions.
	:twitter:card: summary_large_image
	:twitter:site: @exakat
	:twitter:title: Treemap
	:twitter:description: Treemap: Treemaps are a visualization tool used to understand the relative weight of partitions
	:twitter:creator: @exakat
	:twitter:image:src: https://php-dictionary.readthedocs.io/en/latest/_static/logo.png
	:og:image: https://php-dictionary.readthedocs.io/en/latest/_static/logo.png
	:og:title: Treemap
	:og:type: article
	:og:description: Treemaps are a visualization tool used to understand the relative weight of partitions
	:og:url: https://php-dictionary.readthedocs.io/en/latest/dictionary/treemap.ini.html
	:og:locale: en
.. raw:: html

	<script type="application/ld+json">{"@context":"https:\/\/schema.org","@graph":[{"@type":"DefinedTerm","@id":"https:\/\/php-dictionary.readthedocs.io\/en\/latest\/dictionary\/treemap.ini.html","url":"https:\/\/php-dictionary.readthedocs.io\/en\/latest\/dictionary\/treemap.ini.html","name":"Treemap","isPartOf":{"@id":"https:\/\/www.exakat.io\/"},"datePublished":"Fri, 19 Jun 2026 21:24:41 +0000","dateModified":"Fri, 19 Jun 2026 21:24:41 +0000","description":"Treemaps are a visualization tool used to understand the relative weight of partitions","inLanguage":"en-US","potentialAction":[{"@type":"ReadAction","target":["https:\/\/php-dictionary.readthedocs.io\/en\/latest\/dictionary\/Treemap.html"]}]},{"@type":"WebSite","@id":"https:\/\/www.exakat.io\/","url":"https:\/\/www.exakat.io\/","name":"Exakat","description":"Smart PHP static analysis","inLanguage":"en-US"}]}</script>


Treemap
-------

Treemaps are a visualization tool used to understand the relative weight of partitions. It works on file systems, and nested code structures. Each zone may be dimensioned by its weight, and also colored with another indicator.

`Documentation <https://en.wikipedia.org/wiki/Treemapping>`__

See also `PHP Meminfo Treemap <https://findley.dev/projects/php-meminfo-treemap/>`_.

Related : :ref:`Graph <graph>`, :ref:`Flame Graph <flamegraph>`, :ref:`Memory <memory>`, :ref:`Benchmarking <benchmarking>`, :ref:`Performance <performance>`

Related packages : `wikimedia/arc-lamp <https://packagist.org/packages/wikimedia/arc-lamp>`_
